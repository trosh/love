...
function love.update(dt)
    ...
    for ii = 0, 2, 2 do
        for i = 1, 2 do
            if s[1].x == warp[i+ii].x and
               s[1].y == warp[i+ii].y then
                s[1].x = warp[3+2*ii-i-ii].x
                s[1].y = warp[3+2*ii-i-ii].y
                addw(ii)
            end
        end
    end
...
function loader()
	size = 0
    s = {{}}
	add(math.floor(w/2), math.floor(h/2))
    for i = 1, 5 do
        add(s[size].x, s[size].y)
    end
	warp = {{}, {}, {}, {}}
